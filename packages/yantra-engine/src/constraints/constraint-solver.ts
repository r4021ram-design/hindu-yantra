...
import { ISGOSConstraint, ConstraintReport, ConstraintResult, ConstraintDiagnostic } from './types';

export interface ConcurrencyAuditReport {
  conditionISatisfied: boolean; // t3 and t7 share same outer circumcircle
  conditionIISatisfied: boolean; // All apex-base height coincidence pairs satisfied
  conditionIIISatisfied: boolean; // 12 triple intersection points coincide
...
export class ConstraintSolver {
  public static readonly TOLERANCE = 1e-12;
  ...
  public static solve(
    ...
    constraints: readonly ISGOSConstraint[]
  ): ConstraintReport {
   ...
   */
  public static verifyChiodoConstraints(
    triangles: ChiodoTriangleSpec[],
    ...
    apexBasePairs.forEach(pair => {
      if (pair.apexY !== undefined && pair.baseY !== undefined) {
        ...
        if (err > apexBaseMaxError) apexBaseMaxError = err;
      }
    });
    ...
    if (t3 && t7 && triMap['t4']) {
      const line_t3_left = Line2D.fromTwoPoints(t3.leftBase, t3.apex);
      const line_t7_left = Line2D.fromTwoPoints(t7.leftBase, t7.apex);
    ...
    triangles.forEach(t => {
      ...
      if (maxErr > symmetryMaxError) symmetryMaxError = maxErr;
    });
    ...
    if (!conditionISatisfied) overallConfidenceScore -= 10;
    if (!conditionIISatisfied) overallConfidenceScore -= 10;
    if (!conditionIIISatisfied) overallConfidenceScore -= 10;
