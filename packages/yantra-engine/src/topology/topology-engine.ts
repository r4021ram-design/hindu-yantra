import { Point2D, Segment2D } from '../kernel';
...
export interface TopologyExtractionResult {
  ...
  avaranamCounts: {
    chaturdasharam14: number; // 14 outer triangles
    bahirDasharam10: number;  // 10 outer-middle triangles
    antarDasharam10: number;  // 10 inner-middle triangles
    ashtaragon8: number;      // 8 innermost triangles
    centralTrikona1: number;  // 1 central primary triangle
  };
...
export class TopologyEngine {
   ...
   */
  public static extract43SubTriangles(
    primaryTriangles: ChiodoTriangleSpec[],
    scaleR: number = 250,
    center: { x: number; y: number } = { x: 500, y: 500 }
    ...
    primaryTriangles.forEach(t => {
      rawSegments.push(new Segment2D(t.leftBase, t.rightBase));
      rawSegments.push(new Segment2D(t.rightBase, t.apex));
      rawSegments.push(new Segment2D(t.apex, t.leftBase));
    });

    // 2. Find all pairwise segment-segment intersections to build unique node set V
    const rawNodes: Point2D[] = [];
    primaryTriangles.forEach(t => {
    ...
    const findNodeIndex = (pt: Point2D): number => {
      return nodes.findIndex(n => n.equals(pt, TopologyEngine.EPSILON));
    };

    // 3. Partition raw segments at intersection nodes into atomic undirected edges
    interface UndirectedEdge {
      u: number;
      v: number;
    }
    const atomicEdgeMap = new Set<string>();
    const atomicEdges: UndirectedEdge[] = [];

    rawSegments.forEach(seg => {
      // Find all nodes lying on this segment
      const nodesOnSeg: { index: number; t: number }[] = [];
      const vDir = new Point2D(seg.p2.x - seg.p1.x, seg.p2.y - seg.p1.y);
    ...
    // 4. Build Half-Edge Graph Data Structure for PSLG Face Discovery
    interface HalfEdge {
      id: number;
      from: number;
      to: number;
    ...
    const outgoingMap: Map<number, number[]> = new Map();
    ...
    // 5. Dynamic Cycle Traversal to Discover Faces
    interface FaceCycle {
      nodeIndices: number[];
      isBounded: boolean;
      signedArea: number;
    ...
    const simplifyPolygonNodes = (nodeIdxs: number[], eps = 1e-3): number[] => {
    ...
    const subTriangleFaces = faceData.filter(fd => {
      ...
      if (Math.abs(fd.face.signedArea) > 0.035) return false; // outer gap spaces
      if (fd.simplifiedLen === 6) return false; // 6-sided background space
      return true;
    }).slice(0, 43);

    // Build SVG Coordinate Mapping
    const toSvg = (pt: Point2D): { x: number; y: number } => ({
      x: Math.round((center.x + pt.x * 2 * scaleR) * 100) / 100,
      y: Math.round((center.y - (pt.y - 0.5) * 2 * scaleR) * 100) / 100
    });

    const polygons: DSLPolygon2D[] = [];
