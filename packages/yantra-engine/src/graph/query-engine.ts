...
export class GraphQueryEngine {
  ...
  public findAllTriangles(): readonly GraphFace[] {
    return Object.values(this.graph.faces).filter(
      f => f.type === 'sub_triangle' || f.nodeIds.length === 3
    );
  }

  /** Find all lotus petal faces in graph */
  public findAllPetals(): readonly GraphFace[] {
    return Object.values(this.graph.faces).filter(f => f.type === 'lotus_petal');
  }

  /** Find all sacred enclosure regions (Navavarana Chakras) */
  public findAllRegions(): readonly GraphRegion[] {
    return this.graph.regions;
  }

  /** Find all faces sharing common edges, vertex nodes, or centroids with a given face */
  public findNeighboringFaces(faceId: string): readonly GraphFace[] {
    ...
    const neighbors: GraphFace[] = [];

    for (const f of Object.values(this.graph.faces)) {
      if (f.id === faceId) continue;
  ...
  public findOuterBoundary(): readonly GraphEdge[] {
