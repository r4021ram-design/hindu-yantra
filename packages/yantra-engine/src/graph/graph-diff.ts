...
export class GraphDiffEngine {
   ...
   */
  public static compare(graphA: GeometryGraph, graphB: GeometryGraph): GraphTopologyDiff {
    ...
    const modifiedLayers = Array.from(new Set([...layersA, ...layersB])).filter(
      id => !layersA.has(id) || !layersB.has(id)
    );
    ...
    const structuralDeltas: string[] = [];
    if (graphA.tradition !== graphB.tradition) {
      structuralDeltas.push(
        `Tradition changed from "${graphA.tradition}" to "${graphB.tradition}".`
      );
    }
    if (graphA.symmetryGroup !== graphB.symmetryGroup) {
      structuralDeltas.push(
        `Symmetry group changed from "${graphA.symmetryGroup}" to "${graphB.symmetryGroup}".`
      );
    }
