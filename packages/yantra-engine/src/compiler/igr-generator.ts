import {
  IntermediateGeometryRepresentation,
  IGRNode,
  IGREdge,
  IGRLayer,
  IGRConstraint,
...
export class IGRGenerator {
  public static readonly COMPILER_VERSION = '1.0.0-sgos.phase1';
  ...
  public static generate(
    dsl: any,
    constraints: readonly IGRConstraint[],
    graph: ConstructionGraph,
    diagnostics: CompilerDiagnosticsBuilder
  ): IntermediateGeometryRepresentation {
    const nodes: Record<string, IGRNode> = {};
    const edges: IGREdge[] = [];
    const layers: IGRLayer[] = [];
    ...
    if (Array.isArray(dsl.geometryRules?.lotusRings)) {
      dsl.geometryRules.lotusRings.forEach((lotus: any, idx: number) => {
        lotusRingCount++;
    ...
    if (Array.isArray(dsl.geometryRules?.concentricCircles)) {
      dsl.geometryRules.concentricCircles.forEach((circle: any, idx: number) => {
        circleCount++;
