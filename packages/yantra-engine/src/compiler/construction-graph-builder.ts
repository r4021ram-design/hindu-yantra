import { ConstructionGraph, ConstructionGraphNode } from './types';
import { CompilerDiagnosticsBuilder } from './diagnostics';

export class ConstructionGraphBuilder {
  /**
   * Pipeline Stage 4 & 5: Build Construction Graph DAG & Validate Topology (Detect Cycles)
   */
  public static build(dsl: any, diagnostics: CompilerDiagnosticsBuilder): ConstructionGraph {
    const rawNodes: ConstructionGraphNode[] = [];
    ...
    if (dsl.geometryRules?.concentricCircles?.length) {
      rawNodes.push({
        ...
        layerId: 'mekhala',
        primitiveIds: dsl.geometryRules.concentricCircles.map((c: any) => c.id),
        dependsOnStepIds: [prevStepId],
  ...
  private static detectCycles(nodes: ConstructionGraphNode[], diagnostics: CompilerDiagnosticsBuilder): boolean {
      ...
      if (!visited.has(node.id)) {
        if (dfs(node.id)) {
          diagnostics.addError(
            'SGOS_E020_CYCLIC_DEPENDENCY',
            `Cyclic dependency detected in construction graph step "${node.id}".`,
            { path: 'constructionGraph' },
            'Ensure construction step dependencies form a Directed Acyclic Graph (DAG).'
          );
          return true;
        }
      }
