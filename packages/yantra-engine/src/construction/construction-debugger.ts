...
import { ChiodoConstructionEngine, ChiodoTriangleSpec, ChiodoConstructionResult } from './chiodo-construction-engine';
...
export interface ConstructionStepTrace {
  ...
  theorem: string;
  equation: string;
  newPoints: { id: string; x: number; y: number; label: string }[];
  newLines: { id: string; d: string; stroke: string; equation: string }[];
  newCircles: { id: string; cx: number; cy: number; r: number }[];
  dependencyNodes: string[];
  activeTriangles: string[]; // e.g. ['t1', 't3', 't7']
}
...
export interface TriangleCoordinatesExport {
  ...
  baseMidpoint: { x: number; y: number };
  baseHeightY: number;
  baseWidth: number;
}

export class ConstructionDebugger {
  private traces: ConstructionStepTrace[] = [];
  private construction: ChiodoConstructionResult;

  constructor(inputParams: Partial<Parameters<typeof ChiodoConstructionEngine.construct>[0]> = {}) {
    this.construction = ChiodoConstructionEngine.construct(inputParams);
   ...
   */
  private buildAll80StepTraces(): void {
    const derived = ChiodoConstructionEngine.deriveChiodoBaseParameters();
    const apollonius = ApolloniusSolver.solveChiodoCLP(derived);
    const tris = this.construction.primaryTriangles;
    const triMap: Record<string, ChiodoTriangleSpec> = {};
  ...
  public getStep(stepNumber: number): ConstructionStepTrace {
   ...
   */
  public getTriangleCoordinates(): TriangleCoordinatesExport[] {
  ...
  public renderStepSVG(
    ...
    this.construction.primaryTriangles.forEach(t => {
      if (!visibleTris.has(t.id)) return;
      const stepForTri = parseInt(t.id.replace('t', ''), 10) + 5;
      if (stepNumber < stepForTri && stepNumber > 5) return;
   ...
   */
  public findFirstDivergenceStep(referenceVertices: { id: string; x: number; y: number }[]): { firstDivergenceStep: number | null; maxDeviation: number; details: string } {
    const coords = this.getTriangleCoordinates();
    let maxDev = 0;
    let divStep: number | null = null;
    ...
    referenceVertices.forEach(ref => {
      ...
      if (genTri) {
        let genPt = { x: 0, y: 0 };
        if (part === 'left') genPt = genTri.leftBase;
        else if (part === 'right') genPt = genTri.rightBase;
        else if (part === 'apex') genPt = genTri.apex;
