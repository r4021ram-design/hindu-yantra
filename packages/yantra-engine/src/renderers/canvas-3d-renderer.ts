...
import { Mesh3DGenerator, YantraMaterialType } from './mesh-3d-generator';
import { SGOSSceneGraph, SGOSSceneGraphEngine } from './scene-graph';
...
export interface Canvas3DRenderOptions {
  rotX?: number; // Pitch angle in radians
  rotY?: number; // Yaw angle in radians
  material?: YantraMaterialType;
  mode?: 'flat_relief' | 'maha_meru_pyramid';
  wireframe?: boolean;
  xrayMode?: boolean;
  explodedViewOffset?: number; // Z-explosion separation distance
  crossSectionY?: number; // Clipping plane height cutoff
...
export class Canvas3DRenderer {
  ...
  public static renderToCanvas(
    ...
    width: number,
    height: number,
    target: YantraDSL | CompiledGeometryModel | OptimizedSolvedGeometryModel | SGOSSceneGraph,
    options: Canvas3DRenderOptions = {}
  ) {
    let sceneGraph: SGOSSceneGraph;
    if ('root' in target) {
      sceneGraph = target as SGOSSceneGraph;
    } else if ('sgm' in target) {
      sceneGraph = SGOSSceneGraphEngine.fromOSGM(target);
    } else {
      const compiled = 'geometryRules' in target ? GeometryCompiler.compile(target as YantraDSL) : (target as CompiledGeometryModel);
      sceneGraph = SGOSSceneGraphEngine.fromOSGM({ dslId: compiled.yantraId, solvedCoordinates: {}, solvedFaces: {}, solvedEdges: {} } as any);
    }
