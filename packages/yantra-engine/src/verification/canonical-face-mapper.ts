...
export interface CanonicalFaceMapping {
  canonicalIndex: number; // 1 to 43
  faceId: string;
  navavaranaNumber: number; // 1 to 9
  sanskritName: string;
  englishName: string;
  avaranaName: string;
...
export class SGOSCanonicalFaceMapper {
  /**
   * Maps all generated faces to their canonical 43 indices, 9 Navavaranas, and Shastric classifications.
   */
  public static mapFaces(dsl: GeometryDSL): CanonicalFaceMappingReport {
    const compiled = GeometryCompiler.compile(dsl as any);

    const mappings: CanonicalFaceMapping[] = [];
    const navDistribution: Record<number, number> = {
      1: 0, // Bhupura
      2: 0, // 16 Lotus
      3: 0, // 8 Lotus
      4: 0, // 14 Triangles
    ...
    return {
      ...
      mappings: Object.freeze(mappings) as any,
