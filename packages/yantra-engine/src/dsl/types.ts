...
export interface NavavaranaSpec {
  avaranaNumber: number; // 1 to 9
  sanskritName: string;
  englishName: string;
  chakraName: string; // e.g. Trailokya Mohana, Sarvasaparipuraka
...
export interface YantraDSL {
  ...
  layers?: any;
  construction?: {
    primaryTrianglesCount: number; // 9 for Shri Yantra
    subTrianglesCount: number; // 43 for Shri Yantra
...
export interface SolverOutput {
  ...
  polygons: Polygon2D[];
  circles: { id: string; cx: number; cy: number; r: number; layerId: string }[];
  paths: { id: string; d: string; layerId: string; fill?: string; stroke?: string; strokeWidth?: number }[];
  points: (Point2D & { id: string; layerId: string })[];
  layerNames: string[];
  metrics?: any;
