...
class SGOSASTParser {
    ...
    static parse(rawDsl) {
        const errors = [];
        const symbolTable = new symbol_table_1.SymbolTable();
        ...
        const childrenNodes = [];
        // 1. Metadata Node
        const metaNode = ast_nodes_1.ASTNodeFactory.createMetadataNode({
        ...
        // 2. Bindu Node
        const binduNode = ast_nodes_1.ASTNodeFactory.createBinduNode();
        const binduLayer = ast_nodes_1.ASTNodeFactory.createLayerNode({
            layerId: 'bindu',
            sanskritName: 'Bindu',
        ...
        if (rules.shriYantraCore || rules.triangleSets || dslId === 'sri_yantra') {
            const shivaNode = ast_nodes_1.ASTNodeFactory.createTriangleNode({
                triangleId: 'shiva_primary_group',
                orientation: 'upward',
                isPrimary: true
            });
            const shaktiNode = ast_nodes_1.ASTNodeFactory.createTriangleNode({
                triangleId: 'shakti_primary_group',
                orientation: 'downward',
                isPrimary: true
            });
            const coreLayer = ast_nodes_1.ASTNodeFactory.createLayerNode({
        ...
        if (Array.isArray(rules.lotusRings)) {
            rules.lotusRings.forEach((lotus, idx) => {
                ...
                const lotusNode = ast_nodes_1.ASTNodeFactory.createLotusNode({
                    lotusId,
                ...
                const lotusLayer = ast_nodes_1.ASTNodeFactory.createLayerNode({
                    layerId: lotusId,
        ...
        if (Array.isArray(rules.concentricCircles)) {
            rules.concentricCircles.forEach((circle, idx) => {
                ...
                const circleNode = ast_nodes_1.ASTNodeFactory.createCircleNode({
                    circleId,
                    radiusRatio: circle.radiusRatio ?? 0.8,
                    sanskritName: circle.sanskritName
                });
                const circleLayer = ast_nodes_1.ASTNodeFactory.createLayerNode({
                    layerId: circleId,
        ...
        if (rules.bhupura?.enabled) {
            const bhupuraNode = ast_nodes_1.ASTNodeFactory.createBhupuraNode({
                enabled: true,
                steps: rules.bhupura.steps || 3,
                gateCount: 4
            });
            const bhupuraLayer = ast_nodes_1.ASTNodeFactory.createLayerNode({
                layerId: 'bhupura',
        ...
        // Construct Root Node
        const rootNode = ast_nodes_1.ASTNodeFactory.createRootNode({
            dslId,
            tradition,
...
exports.SGOSASTParser = SGOSASTParser;
