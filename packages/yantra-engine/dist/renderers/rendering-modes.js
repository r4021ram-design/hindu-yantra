...
exports.YantraModeRenderer = void 0;
...
class YantraModeRenderer {
     ...
     * 4. 'topology'
     */
    static renderMode(compiled, mode, options = {}) {
        const scaleR = 400;
        const cx = 500;
        const cy = 500;
        ...
        const constr = chiodo_construction_engine_1.ChiodoConstructionEngine.construct();
        const apollonius = apollonius_solver_1.ApolloniusSolver.solveChiodoCLP();
        ...
        if (mode === 'primary_triangles') {
            ...
            const getLineY = (lineNum) => Math.round((startY + (lineNum - 1) * stepY) * 100) / 100;
            ...
            const mapX = (x) => Math.round((500 + x * 900) * 100) / 100;
            const mapY = (y) => Math.round((950 - y * 900) * 100) / 100;
            solution.primaryTriangles.forEach((t) => {
     ...
     */
    static generateSplitScreenComparison(compiled) {
...
exports.YantraModeRenderer = YantraModeRenderer;
