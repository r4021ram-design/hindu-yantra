...
const rendering_strategies_1 = require("./rendering-strategies");
class SVGRenderer {
    /**
     * Render a CompiledGeometryModel, YantraDSL, SGOSSceneGraph, or OSGM into a complete SVG string
     */
    static renderToString(target, options = {}) {
        let compiled;
        if ('root' in target) {
            const dsl = yantras_dsl_1.MASTER_YANTRA_DATASET.find(y => y.id === target.yantraId) || yantras_dsl_1.MASTER_YANTRA_DATASET[0];
            compiled = compiler_1.GeometryCompiler.compile(dsl);
        }
        else if ('sgm' in target) {
            const dsl = yantras_dsl_1.MASTER_YANTRA_DATASET.find(y => y.id === target.sgm.dslId) || yantras_dsl_1.MASTER_YANTRA_DATASET[0];
            compiled = compiler_1.GeometryCompiler.compile(dsl);
        }
        else if ('geometryRules' in target) {
            compiled = compiler_1.GeometryCompiler.compile(target);
        }
        else {
            compiled = target;
        }
        const theme = options.theme || 'gold';
        const strategy = rendering_strategies_1.YantraRenderingStrategyFactory.getStrategy(theme);
        if (strategy) {
    ...
    static getAvaranaIndexFromLayerId(layerId) {
        switch (layerId) {
