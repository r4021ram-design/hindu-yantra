...
class GeometryCompiler {
    ...
    static compile(dsl) {
        ...
        if (dsl.geometryRules.triangleSets && dsl.geometryRules.triangleSets.length > 0) {
            for (const tSet of dsl.geometryRules.triangleSets) {
                ...
                if (tSet.interlocking && (dsl.id.includes('sri') || dsl.id.includes('meru'))) {
                    ...
                    const construction = chiodo_construction_engine_1.ChiodoConstructionEngine.construct(chiodo_apollonius_solver_1.ChiodoApolloniusSolver.DEFAULT_PARAMETERS);
                    const topology = topology_engine_1.TopologyEngine.extract43SubTriangles(construction.primaryTriangles, sriScaleR, { x: this.CENTER, y: this.CENTER });
        ...
        model.validationReport = validator_1.GeometryValidator.validate(dsl, model);
        return model;
    }
    /**
     ...
     * t9 = Black (#000000)
     */
    static compileShriPrimaryLinesWithDistinctColors(construction, scaleR) {
     ...
     */
    static compileKuberMagicSquareGrid(size) {
        const cx = this.CENTER;
        const cy = this.CENTER;
        const half = size / 2;
        const paths = [];
     ...
     */
    static compileNavagrahaOrbitalSpheres(orbitR) {
        const cx = this.CENTER;
        const cy = this.CENTER;
        const circles = [];
        const points = [];
     ...
     */
    static compileVastuPurusha81Grid(size) {
        const cx = this.CENTER;
        const cy = this.CENTER;
        const half = size / 2;
        const step = size / 9;
     ...
     */
    static compileBhupuraPath(steps, radius) {
        let d = '';
        const cx = this.CENTER;
        const cy = this.CENTER;
     ...
     */
    static compileLotusRingPath(petalCount, innerR, outerR, shape) {
        let d = '';
        const cx = this.CENTER;
        const cy = this.CENTER;
     ...
     */
    static compileStarPolygon(points, radius) {
        const cx = this.CENTER;
        const cy = this.CENTER;
        const paths = [];
        if (points === 6) {
     ...
     */
    static compileGenericTriangles(count, baseR, apexR, direction) {
        const cx = this.CENTER;
        const cy = this.CENTER;
        const result = [];
...
exports.GeometryCompiler = GeometryCompiler;
