...
exports.GeometryValidator = void 0;
...
class GeometryValidator {
     ...
     */
    static validate(dsl, compiled) {
        const isShri = dsl.id.includes('sri') || dsl.id.includes('shri') || dsl.id.includes('meru');
        const chiodoAudit = isShri ? chiodo_validator_1.ChiodoValidator.audit() : null;
        const checks = [];
        // Rule 1: Centroid & Bindu Alignment Check
        const binduCircle = compiled.circles.find((c) => c.id === 'bindu');
...
exports.GeometryValidator = GeometryValidator;
